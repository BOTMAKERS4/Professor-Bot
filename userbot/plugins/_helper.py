...
async def _(event):
    ...
    if tgbotusername is not None or mafia_input == "text":
        results = await event.client.inline_query(tgbotusername, "@harshjais369")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of ProfessorBot\n\n"
        )
        mafiacount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the ProfessorBot are [here]({url})"
