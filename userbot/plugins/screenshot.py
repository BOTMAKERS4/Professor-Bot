...
async def _(event):
    ...
    if Config.GOOGLE_CHROME_BIN is None:
        await edit_or_reply(event, "Need to install Google Chrome. Module is stopping...")
        return
    ...
    try:
        ...
        await edit_or_reply(event, "Calculating page's dimensions...")
        ...
        with io.BytesIO(im_png) as out_file:
            out_file.name = "ProfessorBot.ScreenCapture.PNG"
        ...
        await edit_or_reply(event, f"Completed screen capturing! Process done in {ms} seconds.")
    except Exception:
        await edit_or_reply(event, traceback.format_exc())

CmdHelp("screenshot").add_command(
  "screenshot", "<link>", "Gives out the web screenshot of given link via Google Crome Bin in .png format", ".screenshot https://github.com/harshjais369/ProfessorBot"
).add()