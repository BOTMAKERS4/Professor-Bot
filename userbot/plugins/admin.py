...
NO_PERM = "`I don't have sufficient permissions! Sed lyf -_-`"
...
async def ban(bon):
    ...
    if not user:
        return
    mafiaevent = await edit_or_reply(bon, "Banning this user...")
    try:
...
async def nothanos(unbon):
    ...
    try:
        ...
        await mafiaevent.edit("Sorry, I can't unban this user!")
...
async def _(event):
    ...
    if c.admin_rights or c.creator:
        ...
        await edit_or_reply(
            event, "You need administrative permissions in order to execute this command."
        )
        await sleep(3)
        try:
            await event.delete()
...
CmdHelp("admin").add_command(
       'setgpic', '<reply to image>', 'Changes the groups display picture.'
        ...
        'pin', '<reply> or .pin loud', 'Pins the replied message in the group.'
).add_command(
        'kick', '<username/reply>', 'Kicks the person from your chat.'
).add_command(
        'iundlt', None, 'Displays last 5 deleted messages in group.'
).add()
